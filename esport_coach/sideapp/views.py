...
def list_of_coaches(request):
    ...
    for i in range(len(user)):
        coachtest.append([user[i].full_name, user[i].mmr, user[i].pricerate, user[i].server, user[i].hero, user[i].reputation, user[i].rating, user[i].students, id])
...
def tutorselected(request, id):
    tutor = Signup.objects.get(pk=id)
    context = {'coach': tutor}
